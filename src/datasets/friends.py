...
class FriendsDialog(Dataset):
    def __init__(
        ...
        with open(self.txt_path, "r", encoding="utf8") as f:
            for line in f:
                ...
                ut1_ids = self.dictionary.tokens2id(
                    ut1_tokens, add_unknown=build_dict
                )

                ut2_tokens = self.dictionary.tokenize(ut2)
                ut2_ids = self.dictionary.tokens2id(
                    ut2_tokens, add_unknown=build_dict
                )
    ...
    def __getitem__(self, index):
        ...
        data = torch.tensor(
            [self.dictionary.bos_id]
            + ut1
            + [self.dictionary.eos_id]
            + [self.dictionary.pad_id] * ut1_pad_len
        )
        target = torch.tensor(
            [self.dictionary.bos_id]
            + ut2
            + [self.dictionary.eos_id]
            + [self.dictionary.pad_id] * ut2_pad_len
        )
        return data, target
